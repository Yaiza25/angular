...
export class ContactoServiceHttp extends ContactoServiceInterface {
    ...
    override agregar(nuevo: ContactoModel): Observable<any> {
        return this.http.post<ContactoModel>(this.url, nuevo);
    }

    override eliminar(id: number): Observable<any> {
        return this.http.delete<ContactoModel[]>(`${this.url}/${id}`);
    }
    
    override modificar(id: number, modificado: ContactoModel): Observable<any> {
        return this.http.put<ContactoModel>(`${this.url}/${id}`, modificado);
    }
}